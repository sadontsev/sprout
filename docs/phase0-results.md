...
> **Secrets are NOT in this file.** The admin password, app API key, camera token, JWT, and HA token live on `<your-server>` under `<secrets-dir>/` (mode 600) and were shown to the owner once. Reference them by file, never commit their values.
...
| | |
|---|---|
| Bambuddy LAN | `http://192.168.1.10:8910` (host-networked on <your-server>) |
...
| App auth | **`X-API-Key`** header (key in `<your-server>:<secrets-dir>/bb_apikey`), scoped: read_status + queue + control + manage_library. Verified: 401 without, 200 with. |
| Admin (web UI only) | user `max`, password in `<your-server>:<secrets-dir>/bb_admin_pw`. JWT login `POST /api/v1/auth/login` (24h, no refresh). Admin-only mutations need JWT; the app's API key covers all operational endpoints. |
...
- **Camera:** `GET /api/v1/printers/1/camera/snapshot?token=<cam-token>` → JPEG **1536×1080** (✅). MJPEG stream at `/camera/stream`. Long-lived camera token in `<your-server>:<secrets-dir>/` (mint via `POST /api/v1/printers/camera/stream-token`). A1 has no plate PNG from headless slice.
- **Slicing (verified):**
...
## 4. Slicer sidecars (on <your-server>)
...
`deploy/bambuddy/` (Bambuddy compose + env example), `deploy/slicer-api/` (slicer sidecars), `deploy/README.md`. Running copies on <your-server>: `<deploy-dir>/bambuddy/`, `<deploy-dir>/slicer-api/`. Cloudflared rule added to `<deploy-dir>/cloudflared/config.yml` (backup `config.yml.bak.20260628-135453`).
