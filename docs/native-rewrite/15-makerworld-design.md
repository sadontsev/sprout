...
| Key in `<secrets-dir>/bb_apikey` | matches prefix of **key 1, `ios-app`, `can_access_cloud = 0`** |
...
Every probe in the document used the key in `<secrets-dir>/bb_apikey` — key **1**, the
