...
iOS 17+ `URL(string:)` percent-encodes many invalid characters rather than failing, but I confirmed it still returns nil for a stray `]`, `<`/`>`, or a bare `%` in the authority (tested against this toolchain's Foundation: `"https://homeserver.local:8910]/x"` → nil, `"https://hos<t>.local/x"` → nil).
