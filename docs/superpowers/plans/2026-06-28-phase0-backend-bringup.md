...
**Tech Stack:** Docker / Docker Compose, Bambuddy (FastAPI), Bambu Studio CLI sidecar, Home Assistant REST/WebSocket API + Mosquitto MQTT, Tailscale, `curl`/`jq`/`openssl` for verification. All ops run from this laptop over SSH to `max@<your-server>`.
...
- **Host `<your-server>`:** Linux **x86_64** (slicer images are linux/amd64-only — confirmed compatible), LAN `192.168.1.10`, Tailscale `100.100.100.100`. Docker present. SSH as `max@<your-server>`.
...
- `GEM_TS_HOST` — <your-server>'s MagicDNS name from `tailscale status --json` (e.g. `<your-server>-1.<tailnet>.ts.net`).
...
Created in **this repo** (infra-as-code; the running copies live on <your-server>):
...
- `deploy/README.md` — how to deploy/update both stacks on <your-server>.
...
## Task 1: Pre-flight — confirm the A1 is reachable from <your-server> and in Developer Mode

**Files:** none (verification only).

**Interfaces:**
- Produces: confirmation that MQTT 8883, FTPS 990, and camera 6000 are open on `192.168.1.20` from <your-server>, and the working `ACCESS_CODE`.
...
- [ ] **Step 2: Verify the three local ports are open from <your-server>**

Run:
```bash
ssh max@<your-server> 'for p in 8883 990 6000; do nc -z -w3 192.168.1.20 $p && echo "OPEN $p" || echo "CLOSED $p"; done'
```
...
- [ ] **Step 3: Verify MQTT TLS + access code auth from <your-server>** (read-only status subscribe; uses the access code, disables cert check for the self-signed printer cert):

Run:
```bash
ssh max@<your-server> "docker run --rm eclipse-mosquitto:2 mosquitto_sub \
...
# linux/amd64 only (<your-server> is x86_64 — OK).
services:
  orca-slicer-api:
...
## Task 4: Deploy the slicer sidecar on <your-server> and verify it slices

**Files:** none in repo (deploys Task 2 files to <your-server>).

**Interfaces:**
- Produces: a healthy `bambu-studio-api` on `<your-server>:3001` that returns a `.gcode.3mf` for a test STL.

- [ ] **Step 1: Copy the slicer compose to <your-server> and start it**

Run:
```bash
ssh max@<your-server> 'mkdir -p <deploy-dir>/slicer-api'
scp deploy/slicer-api/docker-compose.yml max@<your-server>:<deploy-dir>/slicer-api/
scp deploy/slicer-api/.env.example max@<your-server>:<deploy-dir>/slicer-api/.env
ssh max@<your-server> 'cd <deploy-dir>/slicer-api && docker compose --profile bambu up -d'
```
...
Run:
```bash
ssh max@<your-server> 'curl -fs http://localhost:3001/health && echo " <- bambu-studio OK"; curl -fs http://localhost:3003/health && echo " <- orca OK"'
```
Expected: a JSON health body from each + the `OK` echoes.

- [ ] **Step 3: Smoke-test a real slice** (download a public benchmark STL on <your-server>, slice it via Bambu Studio with bundled A1 profiles):

Run:
```bash
ssh max@<your-server> 'curl -fsSL -o /tmp/3dbenchy.stl https://raw.githubusercontent.com/CreativeTools/3DBenchy/master/Single_part_DABS/3DBenchy.stl && \
...
**Files:** none in repo (HA change on <your-server>, reversible).
...
## Task 6: Deploy Bambuddy on <your-server> and register the A1 as its sole client

**Files:** none in repo (deploys Task 3 files to <your-server>).

**Interfaces:**
- Consumes: slicer at `:3001` (Task 4); `ACCESS_CODE`; ha-bambulab disabled (Task 5).
- Produces: Bambuddy healthy on `<your-server>:8000`, A1 registered, live telemetry flowing.

- [ ] **Step 1: Generate the JWT secret and write <your-server>'s `.env`**

Run:
```bash
ssh max@<your-server> 'mkdir -p <deploy-dir>/bambuddy'
scp deploy/bambuddy/docker-compose.yml max@<your-server>:<deploy-dir>/bambuddy/
ssh max@<your-server> 'cd <deploy-dir>/bambuddy && \
...
Run:
```bash
ssh max@<your-server> 'cd <deploy-dir>/bambuddy && docker compose up -d && sleep 8 && \
...
```bash
ssh -N -L 18000:localhost:8000 max@<your-server> &
...
**Files:** none in repo (Bambuddy + Mosquitto config on <your-server>).
...
**Files:** none in repo (tailscale config on <your-server>).

**Interfaces:**
- Produces: `https://$GEM_TS_HOST/` serving Bambuddy, reachable from the iPhone over the tailnet (satisfies iOS ATS).

- [ ] **Step 1: Get <your-server>'s MagicDNS name and enable HTTPS serve**

Run:
```bash
ssh max@<your-server> 'tailscale status --json | jq -r ".Self.DNSName" && sudo tailscale serve --bg 8000'
```
Expected: the MagicDNS name (e.g. `<your-server>-1.<tailnet>.ts.net.`) and serve confirms `https://<name>/ proxied to http://127.0.0.1:8000`. (Requires HTTPS enabled in the tailnet admin → Features → HTTPS Certificates.)
...
Run:
```bash
ssh max@<your-server> 'docker inspect cloudflared 2>/dev/null | jq -r ".[0].Config.Env[]" 2>/dev/null | grep -i ingress || echo "checked: no bambuddy ingress"; echo "(verify the CF dashboard has no bambuddy hostname)"'
```
...
Run:
```bash
ssh max@<your-server> 'for i in $(seq 1 60); do \
...
Run:
```bash
ssh max@<your-server> 'docker inspect bambuddy --format "{{.RestartCount}} restarts; started {{.State.StartedAt}}"'
```
...
- Bambuddy base URL (app): `https://<<your-server>-magicdns>/` — tailnet only, HTTPS via `tailscale serve`.
...
- WebSocket: `wss://<<your-server>-magicdns>/api/v1/ws` (mint ws-token via `/api/v1/auth/ws-token`).
...
- **Runtime variables vs. placeholders:** bracketed values (`$PID`, `<<your-server>-magicdns>`, profile names) are runtime-discovered with explicit capture commands, not content gaps. Where Bambuddy's exact request schema may vary by version, the plan says to read `/openapi.json` and adjust — not to guess.
