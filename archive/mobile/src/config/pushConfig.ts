...
/**
 * The effective Trellis base URL the app should register with — or null for LOCAL-ONLY mode.
 *
 * Push architecture: each person self-hosts their OWN Trellis next to their OWN Bambuddy (it polls
 ...
 *  - SERVER (this returns a URL): the app registers each card's push token with Trellis, so the
 ...
 * anyone whose Trellis host differs should just set the explicit URL). null when nothing resolves.
 */
