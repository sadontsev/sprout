...
// The self-contained STL viewer page.
//
// In `Domain/` beside `ViewerJS` and `LayerPage`, because it is a STRING BUILDER — no view, no
// platform — and both the iOS overlay and the macOS viewer window (1g) host the same page. It was
// filed inside `StlViewerOverlay.swift`, which is `#if os(iOS)`, so the Mac host could not build it.
