...
Read the relevant sections when directed by [AGENTS.md](AGENTS.md). Paths remain repository-root relative. Detailed failure history is retained; use the current code and actual test discovery rather than historical case counts.

- [The app (`native/`)](#the-app-native)
- [`native/` is TWO destinations — iOS and macOS, one target](#native-is-two-destinations--ios-and-macos-one-target)
- [MakerWorld is a PAGE, not a sheet](#makerworld-is-a-page-not-a-sheet)
- [`laPushUrl` vs `resolvePushUrl`](#lapushurl-vs-resolvepushurl)
- [Printing more than one filament](#printing-more-than-one-filament)
- [The push relay (`canopy/`)](#the-push-relay-canopy)
- [Shipping](#shipping)
- [Releasing Trellis (`deploy/trellis/`)](#releasing-trellis-deploytrellis)
- [Linting](#linting)
- [Apple platform rules: check, never recall](#apple-platform-rules-check-never-recall)
- [The recurring bug in this codebase: offering what the backend will refuse](#the-recurring-bug-in-this-codebase-offering-what-the-backend-will-refuse)
- [Bambuddy auth quirks](#bambuddy-auth-quirks)
- [Testing](#testing)
...
There is no SwiftLint. Semantic lint and iOS/device checks remain local; macOS and service PR CI are described in AGENTS.md. The script wraps the `swift-format` that ships in the Xcode
