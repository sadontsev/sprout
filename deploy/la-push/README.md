...
```bash
mkdir -p ~/docker/la-push && cd ~/docker/la-push
...
printf 'BAMBUDDY_API_KEY=%s\n' "$(tr -d '[:space:]' < ~/.config/bambu-phase0/bb_apikey)" >> .env
docker compose up -d --build
curl -s localhost:8911/health
```

Needs your own APNs auth key `.p8` on the host — by default at
`~/.config/bambu-phase0/apns_key.p8`, or wherever `APNS_KEY_FILE` points. `APNS_KEY_ID`,
