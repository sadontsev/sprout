...
and it is async because this client sits in the poll loop: a synchronous HTTP
call there would block the event loop on every push.
"""
...
Transport = Callable[[str, str, Optional[dict], Optional[str]], Awaitable[tuple[int, Any]]]
...
class CanopyClient:
    ...
    async def enroll(self, invite_code: str = "", recovery_code: str = "") -> Credentials:
        ...
        status, payload = await self._call("POST", "/v1/enroll", body, authed=False)
        if status != 201:
    ...
    async def challenge(self, purpose: str) -> str:
        status, payload = await self._call("POST", "/v1/challenges", {"purpose": purpose})
        if status != 201:
            raise CanopyError(f"challenge failed: {status} {_reason(payload)}")
        return payload["challenge"]

    async def vouch(self, token: str, apns_environment: str) -> None:
    ...
    async def claim(self, claim_body: dict) -> ClaimResult:
        ...
        """
        try:
            status, payload = await self._call("POST", "/v1/claims", claim_body)
    ...
    async def push(
        ...
        try:
            status, data = await self._call("POST", "/v1/push", body)
    ...
    async def release(self, token: str) -> bool:
        ...
        first-come claim."""
        status, _ = await self._call("POST", "/v1/bindings/release", {"token": token})
        return status == 204
    ...
    async def delete(self, token: str) -> bool:
        ...
        status, _ = await self._call("DELETE", "/v1/bindings", {"token": token})
        return status == 204

    async def healthy(self) -> bool:
        try:
            status, _ = await self._call("GET", "/v1/health", None, authed=False)
    ...
    async def _call(self, method: str, path: str, body: Optional[dict], authed: bool = True):
        bearer = None
        if authed:
            if self.credentials is None:
                raise CanopyError("not enrolled")
            bearer = self.credentials.bearer
        return await self._transport(method, path, body, bearer)
